...
async function analyzeFile(
  ...
  if (language) {
    try {
      ...
      if (tree) {
        // Both extractors run on the same tree — one parse, two outputs
  ...
  // If AST import extraction failed, fall back to regex import counting
  // (importMap.imports will be empty, but dependencyDepth from regex is used
  //  for the heuristic score — they're independent signals)
  ...
  const fileScore = computeFileScore({
    ...
    isTestFile,
    astMetrics,
    // Phase 4 coupling fields not available in worker — applied by applyGraphData()
    couplingScore: null,
    isInCycle:     null,
  })
  ...
  const metrics: FileDebtMetrics = {
    ...
    isTestFile,
    astMetrics,
    // Phase 4: null until applyGraphData() runs on main thread
    couplingScore:  null,
    dependencyRisk: null,
    isHubFile:      null,
    isInCycle:      null,
