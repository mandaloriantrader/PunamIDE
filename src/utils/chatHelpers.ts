...
// ── Edit Operations (Phase 3: wired to Rust apply_multi_patch + AST validation) ─
...
export async function resolveEditOperations(parsed: ParsedResponse, projectPath: string): Promise<ParsedResponse> {
  ...
  // Lazy-load AST validator (tree-sitter) — non-blocking if unavailable
  let findBestMatchWithAST: typeof import("./astEditValidator").findBestMatchWithAST | null = null;
  let validateSyntaxAfterEdit: typeof import("./astEditValidator").validateSyntaxAfterEdit | null = null;
  try {
    const validator = await import("./astEditValidator");
    findBestMatchWithAST = validator.findBestMatchWithAST;
    validateSyntaxAfterEdit = validator.validateSyntaxAfterEdit;
  } catch { /* tree-sitter unavailable — continue with string matching */ }
  ...
  for (const editOp of parsed.editOperations) {
    ...
    try {
      ...
      for (const pair of editOp.searchReplace) {
        ...
        // ── PRE-EDIT: AST-aware match resolution ──────────────────────────
        // Uses tree-sitter to disambiguate when SEARCH matches multiple locations
        let foundAt = -1;

        if (findBestMatchWithAST) {
          const astMatch = await findBestMatchWithAST(originalContent, editOp.path, pair.search);
          if (astMatch) {
            foundAt = astMatch.startLine;
          }
        }

        // Fallback: plain string matching (for non-TS/JS or if AST fails)
        if (foundAt < 0) {
        ...
        if (foundAt >= 0) {
          ...
          // Last-resort fallback: JS string replacement
  ...
  if (patches.length > 0) {
    try {
      ...
      if (result.success) {
        ...
        // ── POST-EDIT: Syntax validation via tree-sitter ────────────────────
        // Verify each patched file still has valid syntax
        if (validateSyntaxAfterEdit) {
          const syntaxIssues: string[] = [];
          for (const fileResult of result.file_results) {
            try {
              const newContent = await readFile(getProjectFilePath(projectPath, fileResult.path));
              const validation = await validateSyntaxAfterEdit(newContent, fileResult.path);
              if (!validation.valid) {
                const errorSummary = validation.errors.slice(0, 3).map(
                  (e) => `  Line ${e.line}: ${e.message}`
                ).join("\n");
                syntaxIssues.push(`⚠️ ${fileResult.path} has syntax issues after edit:\n${errorSummary}`);
              }
            } catch { /* can't read file back — skip validation */ }
          }
