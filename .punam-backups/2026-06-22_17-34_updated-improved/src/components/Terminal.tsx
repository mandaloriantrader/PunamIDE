...
export default function Terminal({ cwd, onOutputChange, commandToRun, onCommandStarted, onCommandFailed, onRunObservation, onOpenUrl, aiProviders = [], onFixWithAi }: Props) {
          ...
          ? prev.map((session) =>
              session.id === tabId
                ? {
                    ...session,
                    name: shortName(cmd),
                    lines: [...session.lines, ...newSession.lines],
                    running: true,
                    activeCommand: cmd,
                    activeSessionId: null,
        ...
        setSessions((prev) =>
          prev.map((s) =>
            s.id === tabId
              ? {
                  ...s,
                  lines: [...s.lines, makeLine("error", `Error: ${err}`)],
                  running: false,
                  activeCommand: "",
  ...
  const flushBuffer = useCallback(() => {
    ...
    setSessions((prev) => {
      ...
      const next = prev.map((session) => {
        ...
        return { ...session, lines: [...session.lines, ...pending] };
      });
  ...
  useEffect(() => {
    const unlisten = listen<TerminalOutputPayload>("terminal-output", (event) => {
      ...
      streamedOutputRef.current.set(
        session_id,
        `${streamedOutputRef.current.get(session_id) || ""}${line}\n`
      );
      ...
      setSessions((prev) => {
        ...
        return prev.map((s) => {
          if (s.activeSessionId === session_id || s.id === mappedTabId) {
            return { ...s, lines: [...s.lines, termLine] };
          }
          return s;
        });
      });
  ...
  useEffect(() => {
    const unlisten = listen<TerminalStatusPayload>("terminal-status", (event) => {
    ...
    if (activeSession?.activeSessionId) {
        ...
        prev.map((s) =>
          s.id === activeTabId
            ? { ...s, running: false, status: "killed", activeCommand: "", activeSessionId: null,
                lines: [...s.lines, makeLine("warning", "■ Process was stopped by user")] }
            : s
        )
      );
      ...
      try {
        ...
        setSessions((prev) =>
          prev.map((s) =>
            s.id === activeTabId
              ? { ...s, lines: [...s.lines, makeLine("error", `Failed to stop: ${err}`)] }
              : s
          )
        );
      }
