...
/// Internal: store an embedding without Tauri command overhead.
...
/// Internal: search embeddings returning hits directly.
...
pub fn ensure_embeddings_table() -> Result<(), String> {
    ...
    let has_created_at = {
        ...
        stmt.query_map([], |row| row.get::<_, String>(1)).map_err(|e| format!("Check: {}", e))?
            .filter_map(|c| c.ok()).any(|col| col == "created_at")
    };
    if !has_created_at {
