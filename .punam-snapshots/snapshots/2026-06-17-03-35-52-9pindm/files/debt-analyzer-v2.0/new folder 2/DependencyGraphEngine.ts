...
export class DependencyGraphEngine {
  ...
  build(
    ...
    // Maps every known path variant → canonical absolute path
    // e.g. '/proj/src/utils.ts' and '/proj/src/utils' → '/proj/src/utils.ts'
    ...
    for (const map of importMaps) {
      ...
      for (const edge of map.imports) {
        ...
        if (!toNode) {
          // File referenced but not in known set (e.g. generated file)
          toNode = {
            filePath:     canonical,
            imports:      [],
            importedBy:   [],
  ...
  /**
   * Build a map from all path variants to canonical paths.
   * Handles:
   *  - Extension-stripped paths   (/src/utils → /src/utils.ts)
   *  - .tsx variants              (/src/utils → /src/utils.tsx)
   *  - Index file paths           (/src/utils → /src/utils/index.ts)
   */
  ...
  /**
   * Reconcile an extracted import path to a canonical workspace path.
  ...
  private reconcilePath(
    ...
    // Try stripping .ts extension and looking up
    const stripped = normalized.replace(/\.(ts|tsx|js|jsx)$/, '')
