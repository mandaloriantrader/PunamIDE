...
/// `embedding` is a flat Vec<f32> serialized as little-endian bytes.
#[tauri::command]
pub fn embeddings_store(
    ...
    conn.execute(
        "INSERT OR REPLACE INTO embeddings
         (chunk_id, file_path, chunk_text, start_line, end_line, embedding, created_at)
...
/// Search stored embeddings by cosine similarity to a query embedding.
#[tauri::command]
pub fn embeddings_search(
    ...
    for row in rows {
        if let Ok((id, path, text, sl, el, blob)) = row {
            ...
            hits.push((sim, id, path, text, sl, el));
        }
    }
    ...
    let results: Vec<EmbeddingSearchHit> = hits
        .into_iter()
        .take(top_k)
        .map(|(score, chunk_id, file_path, chunk_text, start_line, end_line)| {
            EmbeddingSearchHit {
                chunk_id,
                file_path,
                chunk_text,
...
pub fn embeddings_clear() -> Result<(), String> {
    ...
    conn.execute("DELETE FROM embeddings", [])
        .map_err(|e| format!("Embeddings clear error: {}", e))?;
    Ok(())
}

/// Initialize the embeddings table. Called from db_init() in lib.rs.
pub fn ensure_embeddings_table() -> Result<(), String> {
    ...
    let has_created_at = {
        let mut stmt = conn
            ...
            .map_err(|e| format!("Embeddings migration check: {}", e))?;
        let columns = stmt
            .query_map([], |row| row.get::<_, String>(1))
            .map_err(|e| format!("Embeddings migration check: {}", e))?;
        let mut found = false;
        for column in columns {
            if column.map_err(|e| format!("Embeddings migration check: {}", e))? == "created_at" {
                found = true;
                break;
            }
        }
    ...
    if !has_created_at {
        ...
        .map_err(|e| format!("Embeddings migration error: {}", e))?;
    }
    let now = unix_timestamp_ms();
    conn.execute(
        "UPDATE embeddings SET created_at = ?1 WHERE created_at = 0",
        [now],
    )
    .map_err(|e| format!("Embeddings timestamp migration error: {}", e))?;
    conn.execute_batch(
        "CREATE INDEX IF NOT EXISTS idx_code_embeddings_created
         ON embeddings(created_at DESC);"
    )
    .map_err(|e| format!("Embeddings index init: {}", e))?;
    Ok(())
}
