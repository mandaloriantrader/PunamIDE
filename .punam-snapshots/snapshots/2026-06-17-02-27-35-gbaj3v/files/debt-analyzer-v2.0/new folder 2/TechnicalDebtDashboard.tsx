...
/**
 * Build import maps for the dependency graph.
 * In the worker path the worker already extracted these — this function
 * handles the main-thread fallback (small projects < 50 files) and
 * approximates from file content using the ASTEngine directly.
 */
...
export default function TechnicalDebtDashboard({
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      // Show initial score immediately (before graph)
      ...
      try {
        // Collect import maps from the worker result
        // (stored on analysis via buildResult importMaps param)
        // For now we approximate by re-parsing imports from file content
        // The worker already extracted importMaps and stored them —
        // they're accessible via the analysis.graph placeholder mechanism
        ...
        // Re-enrich analysis with graph data
        analysis = analyzer.applyGraphData(analysis, bundle);

        // Update score with graph-enriched data
        setScore(scorer.score(analysis));

        // Generate refactor plan with graph data
