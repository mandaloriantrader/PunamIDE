...
const SLIDING_WINDOW_TURNS = 8; // increased: context block uses 2 turns, need room for real messages
...
export function assemblePersistentPayload(inputs: ContextInputs): ContextPayload {
  ...
  // Context block as first user message (file snippets + errors)
  if (contextBlock.trim()) {
    contents.push({
      role: "user",
      parts: [{ text: contextBlock }],
    });
    // Acknowledge context so alternating-turn APIs work correctly
    contents.push({
      role: "model",
      parts: [{ text: "Understood. I have read the file context and am ready to help." }],
    });
  }

  // Recent messages — never truncate user messages (the question must arrive intact)
  ...
  // Ensure last turn is always a user turn (required by most LLM APIs)
